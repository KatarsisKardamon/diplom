...
В файле config.py необходимо подставить TOKEN с сайта poiskkino.dev. 
Для этого нужно зайти на сайт https://poiskkino.dev/, нажать на "Получить доступ к API", после этого откроется бот в телеграмме. 
Далее нужно действовать по инструкции бота, чтобы получить Token.
